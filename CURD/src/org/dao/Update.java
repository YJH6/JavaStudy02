...
public class Update {
    public int update(String username,String password){
        ...
        try {
            ...
            //获得连接(Java连接mysql数据库)
            connection = DriverManager.getConnection(URL, USER_NAME, PASSWORD);//发生sql,执行语句
            statement = connection.createStatement();
            String updateSql = "UPDATE yjh.user SET password ='"+password+"' WHERE username = '"+username+"'";
            int flog  = statement.executeUpdate(updateSql);
